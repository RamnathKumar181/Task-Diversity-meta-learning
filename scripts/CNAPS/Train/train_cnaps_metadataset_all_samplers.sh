...
cd .. && python -m src.main --exp_name cnaps_meta_dataset --train --model cnaps --runs 1 --folder $SLURM_TMPDIR/records --task_sampler $SLURM_ARRAY_TASK_ID --dataset meta_dataset --meta-lr 0.001 --num-ways 5 --num-shots 1 --use-cuda --step-size 0.4 --batch-size 16 --num-workers 0 --num-epochs 1 --output-folder ./config/cnaps_meta_dataset/$SLURM_ARRAY_TASK_ID/
