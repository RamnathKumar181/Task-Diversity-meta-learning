...
python -m src.main --exp_name protonet_meta_dataset --train --model protonet --runs 1 --folder $SLURM_TMPDIR/records --meta-lr 0.001 --task_sampler $SLURM_ARRAY_TASK_ID --dataset meta_dataset --num-ways 5 --num-shots 1 --use-cuda --step-size 0.4 --batch-size 32 --num-workers 0 --num-epochs 2 --output-folder ./config/protonet_meta_dataset/$SLURM_ARRAY_TASK_ID/
